...
{
    return [[self stringArrayByRegex:regex] firstObject];
}

- (NSString *)stringByKeyWord:(NSString *)keyword
                   withSuffix:(NSString *)suffix
{
    ...
    return [regex substringWithRange:NSMakeRange(keyword.length, regex.length - (keyword.length + ([suffix isEqualToString:@"$"] ? 0 : suffix.length)))];
}
...
+ (NSString *)stringWithUnicodes:(NSArray *)unicodes
{
    return [unicodes componentsJoinedByString:@""];
}
...
+ (NSString *)getUnicode:(NSString *)string
{
    if (string.length < 2)
    {
        ...
        return 128 > e ? string : 2048 > e ? [StringUtil stringWithUnicodes:@[u(192 | e >> 6), u(128 | 63 & e)]] : [StringUtil stringWithUnicodes:@[u(224 | e >> 12 & 15), u(128 | e >> 6 & 63), u(128 | 63 & e)]];
    }
